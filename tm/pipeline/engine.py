...
class Pipeline:
    ...
    def run(self, ctx: Dict[str, Any], changed_paths: List[Path], matcher: Callable[[str, Path], bool]) -> Dict[str, Any]:
        for rule in self.plan.rules:
            ...
            for step_name in rule.steps:
                ...
                t1 = time.time()
                self.trace_sink(TraceSpan(
                    rule=rule.name, step=spec.name, t0=t0, t1=t1,
