...
def build_plan() -> Plan:
    steps = {
        "validate_profile": StepSpec(
            ...
            reads=["nfInstanceId", "nfType"], writes=[], fn=step_validate_profile
        ),
        "normalize_services": StepSpec(
            name="normalize_services",
            reads=["services[].name"], writes=["services[].name"], fn=step_normalize_services
        ),
        ...
        "stamp_version": StepSpec(
            name="stamp_version",
            reads=["meta.version"], writes=["meta.version"], fn=step_stamp_version
        ),
        "force_status_ok": StepSpec(
            name="force_status_ok",
            reads=["policy.forceAlive"], writes=["status"], fn=step_force_status_ok
        ),
        "dead_code_step": StepSpec(
            name="dead_code_step",
            reads=["unused"], writes=["unused"], fn=step_dead_code
        ),
    }

    rules = [
        # Typical lifecycle: ensure profile validity first
        Rule(name="on_profile_change", triggers=["nfInstanceId", "nfType"], steps=[
            "validate_profile", "stamp_version"
        ]),
        # Services changes -> normalize names then derive status
        Rule(name="on_services_change", triggers=["services[].state", "services[].name"], steps=[
            "normalize_services", "derive_status"
        ]),
        # Manual status touches
        Rule(name="on_status_touch", triggers=["status"], steps=[
            "stamp_version", "derive_status"
        ]),
        # Policy may force status to ALIVE (conflicts with derive_status). Shares trigger head with on_status_touch
        Rule(name="on_policy_force", triggers=["status", "policy.forceAlive"], steps=[
            "force_status_ok"
        ]),
